...
# some more ls aliases. These ship in Ubuntu's default .bashrc, so they stay
# here rather than moving to shared/ -- promoting them would mean shared/
# redefining something the OS already gives me on every Ubuntu box.
alias ll='ls -alF'
alias la='ls -A'
alias l='ls -CF'
...
__conda_setup="$('/home/gaius/miniconda/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/home/gaius/miniconda/etc/profile.d/conda.sh" ]; then
        . "/home/gaius/miniconda/etc/profile.d/conda.sh"
    else
        export PATH="/home/gaius/miniconda/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
